...
MAX_TOOL_ROUNDS = 6
CHECKLIST_STATUS_VALUES = {"completed", "failed", "unavailable"}
...
def extract_request_checklist(user_request: str) -> list[dict[str, str]]:
    """Extract an explicit numbered checklist from the user request.

    Only lines starting with a number (1., 1), 1:) are treated as mandatory
    checklist items. This deliberately avoids turning ordinary prose into
    artificial obligations.
    """
    import re

    items: list[dict[str, str]] = []
    for raw in user_request.splitlines():
        line = raw.strip()
...
def validate_checklist_completion(
    checklist: list[dict[str, str]],
    reported: Any,
    tool_history: list[dict],
) -> tuple[bool, list[str]]:
    """Validate that every mandatory item has a grounded terminal result."""
    if not checklist:
        return True, []
    if not isinstance(reported, list):
        return False, ["checklist_results mancante o non valido"]

    by_id = {str(item.get("item_id", "")): item for item in reported if isinstance(item, dict)}
    actual_signatures = {
        str(item.get("signature")):
        item for item in tool_history if isinstance(item, dict) and item.get("signature")
    }
    errors: list[str] = []
    for required in checklist:
        ...
        if not item:
            errors.append(f"punto {item_id} senza esito")
            continue
        status = str(item.get("status", "")).lower()
        if status not in CHECKLIST_STATUS_VALUES:
            errors.append(f"punto {item_id} con stato non terminale: {status or 'mancante'}")
            continue
        evidence = item.get("evidence_signatures")
        evidence = evidence if isinstance(evidence, list) else []
        grounded = [actual_signatures.get(str(sig)) for sig in evidence]
        grounded = [entry for entry in grounded if entry is not None]
        if status in {"completed", "failed"} and not grounded:
            errors.append(f"punto {item_id} senza evidenza di uno strumento eseguito")
        if status == "completed" and grounded and not any(int(entry.get("exit_code", 1)) == 0 for entry in grounded):
            errors.append(f"punto {item_id} dichiarato completato ma senza strumento riuscito")
        if status == "unavailable" and not str(item.get("summary", "")).strip():
            errors.append(f"punto {item_id} non disponibile senza motivazione")
...
def execute_safe_tool(repo: Path, request: dict) -> dict:
    ...
    elif name == "inspect_path":
        raw_path = str(args.get("path", ""))
        path = resolve_safe_relative(repo, raw_path)
        tool_result = {
            "command": ["internal-inspect-path", raw_path],
            "exit_code": 0 if path.exists() else 2,
            "output": json.dumps({
                "path": raw_path,
                "exists": path.exists(),
    ...
    elif name == "search_file_text":
        raw_path = str(args.get("path", ""))
        needle_text = str(args.get("text", ""))
        path = resolve_safe_relative(repo, raw_path)
        if not needle_text:
            tool_result = {"command": ["internal-search-text", raw_path], "exit_code": 2, "output": "Testo da cercare mancante."}
        elif not path.exists() or not path.is_file():
            tool_result = {"command": ["internal-search-text", raw_path], "exit_code": 2, "output": "File non trovato."}
        else:
            content = safe_read_text(path, 200000)
            lines = [f"{index}: {line}" for index, line in enumerate(content.splitlines(), 1) if needle_text in line]
            tool_result = {
                "command": ["internal-search-text", raw_path, needle_text],
                "exit_code": 0 if lines else 1,
                "output": "\n".join(lines[:50]) if lines else "Testo non trovato.",
            }
    elif name == "run_pb031_gate":
        candidates = [
            [sys.executable, "-m", "pixel_bot.developer.auto_advance_gate"],
            [sys.executable, "src/pixel_bot/developer/auto_advance_gate.py"],
        ]
        attempts: list[dict] = []
        tool_result = {"command": candidates[0], "exit_code": 127, "output": "PB-031 non disponibile."}
        for command in candidates:
            candidate = run_bounded(command, repo, timeout=180, env={"PYTHONDONTWRITEBYTECODE": "1", "PYTHONPATH": str(repo / "src")})
            attempts.append(candidate)
            if candidate.get("exit_code") != 127 and "No module named" not in str(candidate.get("output", "")):
                tool_result = candidate
                break
        tool_result["attempts"] = attempts
    elif name == "run_configured_lint":
        commands: list[list[str]] = []
        pyproject = safe_read_text(repo / "pyproject.toml", 50000) if (repo / "pyproject.toml").exists() else ""
        if "[tool.ruff" in pyproject or "ruff" in pyproject:
            commands.append([sys.executable, "-m", "ruff", "check", "."])
        if "[tool.mypy" in pyproject or "mypy" in pyproject:
            commands.append([sys.executable, "-m", "mypy", "src"])
        if "flake8" in pyproject or (repo / ".flake8").exists():
            commands.append([sys.executable, "-m", "flake8", "."])
        if not commands:
            tool_result = {"command": [], "exit_code": 3, "output": "Nessun linter configurato rilevato; nessun pacchetto installato."}
        else:
            results = [run_bounded(command, repo, timeout=180, env={"PYTHONDONTWRITEBYTECODE": "1"}) for command in commands]
            tool_result = {
                "command": ["configured-lint"],
                "exit_code": 0 if all(item.get("exit_code") == 0 for item in results) else 1,
                "output": compact(results, 12000),
                "results": results,
            }
    else:
...
def autonomous_tool_loop(
    ...
    tool_history: list[dict] = []
    checklist = extract_request_checklist(user_request)
    validation_feedback: list[str] = []
    planner_instructions = """Sei il planner operativo di Pixel Bot MAGI Brain v5.1 con Checklist Enforcement.
Devi decidere se la richiesta puo ricevere una risposta completa oppure se servono informazioni locali.
Quando mancano dati recuperabili in sicurezza, richiedi strumenti allowlisted senza chiedere conferma.

Rispondi ESCLUSIVAMENTE con un singolo oggetto JSON valido, senza markdown.
Per richiedere strumenti:
{"status":"tool_requests","reason":"...","tools":[{"name":"git_status","arguments":{},"checklist_item_ids":["1"]}]}
Per terminare:
{"status":"final","answer":"risposta finale in italiano","checklist_results":[{"item_id":"1","status":"completed","summary":"...","evidence_signatures":["firma esatta dello strumento"]}]}

Stati checklist ammessi: completed, failed, unavailable.
Ogni punto numerato della richiesta e obbligatorio. Non dichiarare final finche ogni punto ha uno stato terminale.
Per completed o failed devi citare almeno una signature reale presente nei risultati strumenti.
Usa unavailable solo quando nessuno strumento consentito puo verificare il punto e spiegane il motivo.

Strumenti consentiti:
- pytest_quiet
- git_status
- git_recent_log {"count":10}
- git_diff_stat
- git_count_ahead
- inspect_file_bytes {"path":"requirements.txt","byte_count":256}
- read_file_excerpt {"path":"README.md","max_chars":5000}
- list_project_files {"max_depth":2}
- inspect_path {"path":".gitattributes"}
- search_file_text {"path":"file.py","text":"testo esatto"}
- run_pb031_gate
- run_configured_lint

Regole:
1. Associa ogni strumento ai checklist_item_ids che aiuta a verificare.
2. Massimo 6 strumenti per turno.
3. Non richiedere modifiche, installazioni, push, commit o comandi arbitrari.
4. Non ripetere strumenti gia eseguiti.
5. Distingui osservazioni e raccomandazioni.
6. Non sostituire una checklist precisa con una sintesi generale.
"""

    for round_index in range(1, MAX_TOOL_ROUNDS + 1):
        input_text = (
            "RICHIESTA UTENTE:\n" + user_request +
            "\n\nCHECKLIST OBBLIGATORIA ESTRATTA:\n" + compact(checklist) +
            "\n\nACTIVE DISCOVERY:\n" + compact(discovery) +
            "\n\nDELIBERAZIONE MAGI INIZIALE:\n" + compact(initial_magi) +
            "\n\nRISULTATI STRUMENTI GIA ESEGUITI:\n" + compact(tool_history) +
            "\n\nERRORI DI VALIDAZIONE PRECEDENTI:\n" + compact(validation_feedback)
        )
        ...
        try:
            ...
            if checklist:
                validation_feedback = ["Risposta planner non JSON; la checklist non puo essere certificata."]
                continue
            return raw, tool_history, round_index

        status = str(decision.get("status", "")).lower()
        if status == "final":
            valid, errors = validate_checklist_completion(checklist, decision.get("checklist_results"), tool_history)
            if not valid:
            ...
            validation_feedback = ["Risposta finale vuota."]
            continue

        if status != "tool_requests":
            validation_feedback = ["Status planner non valido."]
            continue

        tools = decision.get("tools") if isinstance(decision.get("tools"), list) else []
        if not tools:
            validation_feedback = ["Nessuno strumento richiesto e checklist non completata."]
            continue
        ...
        for tool_request in tools[:MAX_TOOLS_PER_ROUND]:
            if not isinstance(tool_request, dict):
                continue
            checklist_item_ids = tool_request.get("checklist_item_ids")
            checklist_item_ids = [str(x) for x in checklist_item_ids] if isinstance(checklist_item_ids, list) else []
            canonical = {"name": tool_request.get("name"), "arguments": tool_request.get("arguments", {})}
            ...
            if any(item.get("signature") == signature for item in tool_history):
                round_results.append({
                    "signature": signature, "name": tool_request.get("name"), "exit_code": 125,
                    "output": "Richiesta duplicata ignorata.", "checklist_item_ids": checklist_item_ids,
                })
                continue
            executed = execute_safe_tool(repo, tool_request)
            executed["round"] = round_index
            executed["signature"] = signature
            executed["checklist_item_ids"] = checklist_item_ids
            round_results.append(executed)
        tool_history.extend(round_results)

    incomplete = [item for item in checklist if not any(item["id"] in result.get("checklist_item_ids", []) for result in tool_history)]
    answer = (
        "Validazione non dichiarata completata: il limite massimo di turni e stato raggiunto. "
        f"Punti senza uno strumento associato: {', '.join(item['id'] for item in incomplete) or 'nessuno, ma manca una certificazione finale valida'}. "
        "Consulta il report del loop per i risultati disponibili."
    )
    return answer, tool_history, MAX_TOOL_ROUNDS
...
def magi_summary(magi: dict, report_path: Path, discovery_path: Path, tool_history: list[dict], rounds: int, final_report: Path | None) -> str:
    ...
    lines = [
        ...
        f"Checklist Enforcement: {sum(len(item.get('checklist_item_ids', [])) > 0 for item in tool_history)} evidenze associate",
...
def main() -> int:
    ...
    try:
        ...
        loop_report.write_text(json.dumps({
            "request": user_request,
            "mandatory_checklist": extract_request_checklist(user_request),
            "rounds": rounds,
